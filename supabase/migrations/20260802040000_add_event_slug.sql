-- Polymarket's public /event/{slug} page uses the parent EVENT's slug, not a
-- market's own `slug` field. For standalone markets these are usually the
-- same, but for grouped/templated markets (e.g. "...ceasefire by...? (2 week
-- pause)" recurring date families) the market's own slug 404s — confirmed
-- live via gamma-api that events[0].slug is the one that actually resolves.
-- Kept as a separate column instead of overwriting `slug`, since `slug` is
-- also used internally (check_market_closed) for market-level gamma-api
-- lookups that require the market's own slug, not the event's.
alter table opportunities add column if not exists event_slug text;
