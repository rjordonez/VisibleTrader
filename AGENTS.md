...
- When opening a PR (into `develop` or `main`) that ships a user-facing change, include a `## Product Update` section in the PR body. This gets swept into a daily digest posted to the team's marketing Discord channel (or an on-demand digest, triggered manually from the Actions tab) — it is not posted immediately on merge.
